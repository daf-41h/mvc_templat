...
class Validation {
  const DEFAULT_VALIDATION_ERROR = [
    ...
    'between' => '%s harus antara %d dan %d karakter',
    'same' => '%s dan %s tidak sama',
    'alphanumeric' => '%s harus diisi huruf dan angka',
    'secure' => '%s jumlah diantara 8 dan 64 character and dan angka, huruf kecil dan besar, dan karakter spesial',
    'unique' => '%s sudah ada',
  ];
  ...
  public function validate(array $data, array $fields, array $messages = []): array {
    ...
    $validation_errors = array_merge(self::DEFAULT_VALIDATION_ERROR, $rule_messages);
    $errors = [];
